...
function SearchParamsProvider({ children }: { children: (params: { searchQuery: string }) => React.ReactNode }) {
...
function SearchParamsFallback() {
  return <div className="opacity-0">Loading search parameters...</div>
}
      ...
      <Suspense fallback={<SearchParamsFallback />}>
        <SearchParamsProvider>
          {(params: { searchQuery: string }) => {
            ...
            // Set search state when search params are available
            React.useEffect(() => {
              if (searchQuery) {
                setSearch(searchQuery)
                // If there's a search query in the URL, apply it immediately
                debouncedFetchFilteredJobs()
              } else if (!search) {
              ...
              <>
                ...
                <div className="mb-4 flex justify-between items-center">
                  <div className="text-gray-600">
                    Showing{" "}
                    {filteredCount > 0 ? `1-${Math.min(currentJobs.length, jobsPerPage)} of ${filteredCount}` : "0"}{" "}
                    jobs
