...
export default function VerifyPage() {
  ...
  const handleSearch = (query: string) => {
    ...
    const filtered = Object.entries(allJobs).reduce(
      (acc, [id, job]) => {
  ...
  const totalPages = Math.ceil(Object.keys(filteredJobs).length / JOBS_PER_PAGE)
  ...
  const getCurrentPageJobs = () => {
    ...
    return jobIds.slice(startIndex, endIndex).reduce(
      (acc, id) => {
  ...
  const verifyJob = async (jobId: string, verified: boolean) => {
    ...
    try {
      ...
      const response = await fetch(`/api/job/verify?jobId=${jobId}&verified=${verified}`, {
        method: "PATCH",
        headers: {
      ...
      if (response.ok) {
        ...
        if (currentPageJobs.length > 1) {
          ...
          const nextJobId = currentPageJobs[currentIndex + 1] || currentPageJobs[currentIndex - 1]
  ...
  const addSkill = () => {
    ...
    if (trimmedSkill && !form.getValues().skills.includes(trimmedSkill)) {
      const currentSkills = form.getValues().skills.filter((skill) => skill.trim() !== "")
    ...
    return (
      <div className="flex items-center justify-center min-h-[70vh]">
        <div className="animate-pulse text-center">
          ...
          <p className="text-muted-foreground">Please wait while we fetch unverified jobs</p>
        </div>
      </div>
    )
  }
  ...
  if (Object.keys(allJobs).length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh]">
        ...
        <p className="text-muted-foreground">There are no jobs waiting for verification.</p>
          ...
          <TabsList className="w-full flex flex-wrap gap-2">
            {Object.entries(getCurrentPageJobs()).map(([jobId, job]) => (
              <TabsTrigger
                ...
                title={job.title}
              >
                {job.title.length > 30 ? `${job.title.substring(0, 15)}...` : job.title}
              </TabsTrigger>
            ))}
          </TabsList>
        </div>
          ...
          <TabsContent key={jobId} value={jobId}>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle className="text-xl">{isEditing ? "Edit Job Details" : "Review Job Details"}</CardTitle>
                <div className="flex gap-2">
                  ...
                  {isEditing && (
                    <Button variant="default" size="sm" onClick={saveChanges} disabled={isSubmitting}>
                      Save
                    </Button>
                  )}
                </div>
              ...
              <CardContent>
                <Form {...form}>
                  <form className="space-y-6">
                          ...
                          <FormItem>
                            ...
                            <FormControl>
                              <div className="flex items-center">
                                ...
                                <Input {...field} disabled={!isEditing || isSubmitting} placeholder="Enter job title" />
                              </div>
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                            ...
                            <Popover>
                              ...
                              <PopoverContent className="w-auto p-0" align="start">
                                <DateCalendar // Change from Calendar to DateCalendar
                                  mode="single"
                                  selected={
                                    field.value instanceof Date ? field.value : new Date(field.value || Date.now())
                                  }
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                          {isEditing && (
                            <div className="flex gap-2">
                              ...
                              />
                              <Button type="button" onClick={addBenefit} disabled={isSubmitting} size="sm">
                                Add
                              </Button>
                            </div>
                          )}
                      ...
                      {!isEditing &&
                        (!form.watch("benefits") ||
                          ...
                          <p className="text-sm text-muted-foreground italic mt-2">No benefits specified</p>
                        )}
                    </div>
                    ...
                    <div>
                      <FormLabel className="block mb-2">Skills Required</FormLabel>
                      <FormControl>
                        <div className="space-y-2">
                          {isEditing && (
                            <div className="flex gap-2">
                              ...
                              />
                              <Button type="button" onClick={addSkill} disabled={isSubmitting} size="sm">
                                Add
                              </Button>
                            </div>
                          )}
                      ...
                      {!isEditing && form.watch("skills").filter((s) => s.trim() !== "").length === 0 && (
                        <p className="text-sm text-muted-foreground italic mt-2">No skills specified</p>
                      )}
              ...
              <CardFooter className="flex justify-between pt-6 border-t">
                {isEditing ? (
                  <Button className="w-full" onClick={saveChanges} disabled={isSubmitting}>
